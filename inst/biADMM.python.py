...
def elk_python(n,p):
    count = -1
    el1 = el2 = np.zeros([np.int(n),np.int(n*(n-1)/2)])
    el1 = np.r_[np.eye(1),np.zeros([int(n)-1,1])]
    for i in range(2,int(n),1):
        el1 = np.c_[el1,np.r_[np.eye(i),np.zeros([int(n)-i,i])]]
    
    count = -1
    for i in range(1,int(n),1):
        temp = np.zeros([int(n),i])
        temp[i,:] = 1
        el2[:,(count+1):(count+i+1)] = temp
        count = count+i
        
    count = -1
    ek1 = ek2 = np.zeros([int(p),np.int(p*(p-1)/2)])
    ek1 = np.r_[np.eye(1),np.zeros([int(p)-1,1])]
    for i in range(2,int(p),1):
        ek1 = np.c_[ek1,np.r_[np.eye(i),np.zeros([int(p)-i,i])]]

    count = -1
    for i in range(1,int(p),1):
        temp = np.zeros([int(p),i])
        temp[i,:] = 1
...
def biADMM_python(X, nu1, nu2, gamma_1, gamma_2, w_l, u_k, kk, phi, niters, tol, output = 1):
